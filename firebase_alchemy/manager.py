...
class AbstractManager(object):
    ...
    def __init__(self, adaptor, model_cls,
        ...
        if validator:
            if (not isinstance(validator, dict)) and \
        ...
        if not key:
            ...
            try:
                if isinstance(self.validator, list):  # a list of keys
                    ...
                    for key in self.validator:
                        if (not (key in payload)) or \
                           (not isinstance(payload[key],
                                           self.validator[key])):
                            raise Exception()
            except Exception:
                raise ValidationError('Wrong payload format: p:{}, v:{}'
                                      .format(payload, self.validator))
        else:
            # only validate for key pair
            if isinstance(self.validator, dict):
                if key in self.validator and \
                   (not isinstance(payload, self.validator[key])):
                    raise ValidationError(
                        'Wrong value format for key. K: {}, format: {}'
                        .format(payload, self.validator[key]))
        ...
        if self.unique_constraints:
            ...
            for c in self.unique_constraints:
                if not (c in model_args):
                    raise Exception('constraints can not be applied: \
                    not in model construction args')
                filter_args[c] = model_args[c]
            if filter_args:
                res = self.adaptor.session.query(self.model_cls).\
...
class SyncManager(AbstractManager):
    ...
    For example: one person in db response to a firebase document
    about its state.
    """
